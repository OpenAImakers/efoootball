...
function TournamentCodesList({ tournamentId, tournamentName }) {
  ...
  // Default to false so the list is closed by default
  ...
  const generatePDF = async (e) => {
    // Prevent event bubbling so clicking the download button doesn't toggle the header
    e.stopPropagation(); 
  ...
  return (
    <div className="card shadow-sm border-0 mb-5">
      {/* Clickable header bar to toggle the open/closed state */}
      <div 
        className="card-header bg-dark text-white fw-bold py-3 d-flex justify-content-between align-items-center"
      ...
      >
        <h5 className="mb-0 d-flex align-items-center gap-2">
          ...
          Team codes to share to players only
        </h5>
      ...
      {/* Conditionally render the body based on isOpen state */}
      {isOpen && (
